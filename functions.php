 ...
 * Theme setup
 */
function miles_developer_portfolio_blog_e62436aa_setup() {
...
add_action( 'after_setup_theme', 'miles_developer_portfolio_blog_e62436aa_setup' );
 ...
 */
function miles_developer_portfolio_blog_e62436aa_enqueue_frontend_styles() {
    ...
    wp_enqueue_style(
        'miles-developer-portfolio-blog-e62436aa-styles',
        get_template_directory_uri() . '/assets/css/styles.css',
        array(),
        $styles_version
    );
}
add_action( 'wp_enqueue_scripts', 'miles_developer_portfolio_blog_e62436aa_enqueue_frontend_styles' );

/**
 * Enqueue theme scripts for frontend only
 */
function miles_developer_portfolio_blog_e62436aa_enqueue_scripts() {
    ...
    if ( file_exists( $script_path ) ) {
        wp_enqueue_script(
            'miles-developer-portfolio-blog-e62436aa-script',
...
add_action( 'wp_enqueue_scripts', 'miles_developer_portfolio_blog_e62436aa_enqueue_scripts' );

/**
 * Remove title support from pages for cleaner editing experience.
 * The homepage content provides its own visual header.
 */
function miles_developer_portfolio_blog_e62436aa_customize_pages() {
    remove_post_type_support( 'page', 'title' );
}
add_action( 'init', 'miles_developer_portfolio_blog_e62436aa_customize_pages' );

/**
 * Add JS class to html element for animation system.
 * Outputs early in head to prevent FOUC on animated elements.
 */
function miles_developer_portfolio_blog_e62436aa_add_js_class() {
    echo '<script>document.documentElement.classList.add("js");</script>';
}
add_action( 'wp_head', 'miles_developer_portfolio_blog_e62436aa_add_js_class', 1 );
